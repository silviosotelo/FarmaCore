...
step_06_install_tools() {
    ...
    # Create WP-CLI cache directories
    ...
    if ! command_exists wp; then
        ...
        # Create WP-CLI config for root
        cat > /root/.wp-cli/config.yml <<'WPCLI'
...
WPCLI
        ...
        # Verify WP-CLI
        if ! WP_CLI_CACHE_DIR=/root/.wp-cli/cache wp --info --allow-root &>/dev/null; then
...
step_07_configure_firewall() {
    ...
    log "Resetting firewall to clean state"
    ufw --force reset
    ...
    # Allow specific ports (use port numbers for compatibility)
...
step_10_install_wordpress() {
    ...
    if [ ! -f "wp-config-sample.php" ]; then
        execute_with_retry \
        "WP_CLI_CACHE_DIR=/root/.wp-cli/cache sudo -u www-data wp core download --locale=es_ES --allow-root" \
    ...
    if [ ! -f "wp-config.php" ]; then
        ...
        WP_CLI_CACHE_DIR=/root/.wp-cli/cache sudo -u www-data wp config create \
        --dbname=master_wp \
        --dbuser=$WP_DB_USER \
        --dbpass=$WP_DB_PASS \
        --dbhost=localhost \
