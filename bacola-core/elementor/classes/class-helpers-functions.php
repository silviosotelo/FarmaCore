...
trait Bacola_Helper
{
    ...
    *
    */
    protected function bacola_query_elementor_controls($post_count = '8', $column = '4', $carousel = 'no')
    {
    ...
    */
    protected function bacola_elementor_product_loop($settings)
    {
		$output = '';
...
protected function bacola_elementor_product_loop1($settings)
{
    $output = '';
    ...
    // Featured products - usando metadata custom "ind_destacado"
    if($settings['featured'] == 'true'){
        $meta_query[] = array(
            'key'     => 'ind_destacado',
            'value'   => 'S',
    ...
    // Agregar meta_query a args si hay condiciones
    if (!empty($meta_query)) {
        $meta_query['relation'] = 'AND'; // Todas las condiciones deben cumplirse
        $args['meta_query'] = $meta_query;
    }
    //echo 'Args: ' . json_encode($args, true);
    $loop = new \WP_Query( $args );
    //echo 'Products: ' . json_encode($loop, true);
